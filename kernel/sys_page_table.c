...
void print_pte_entry(int level, int index, pte_t *pte) {
...
void print_pagetable(pagetable_t pagetable, int level, uint64 buf_st, int len, int flags) {
    ...
    for (int i = 0; i < 512; i++) {
        ...
        if ((*pte & PTE_V) && (level > 0 || flag_validation)) {
            ...
            print_pagetable((pagetable_t)PTE2PA(*pte), level - 1, buf_st, len, flags);
        }
